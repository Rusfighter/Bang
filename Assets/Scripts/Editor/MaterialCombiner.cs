...
public class MaterialCombiner : EditorWindow
{
    ...
    int m_MaxVerticesPerObject = 33000;
    Vector3 m_GridSize = new Vector3(1, 1, 2);
    GameObject m_Target;
    bool m_DecalCombiner;

    GameObject m_decalMesh;
    GameObject m_surfaceMesh;
    Shader m_decalShader;
    UVChannels m_UVChannel = UVChannels.UV2;

    public void EnableAllMf(GameObject obj)
    {
        obj.SetActiveRecursively(true);
    }

    public GameObject DeepCloneMeshes(GameObject obj)
    {
        GameObject clone = new GameObject(obj.name + "_clone");
    ...
    [MenuItem("Window/Mesh Tools")]
    public static void ShowWindow()
    {
        //Show existing window instance. If one doesn't exist, make one.
        EditorWindow.GetWindow(typeof(MaterialCombiner));
    }
    ...
    void OnGUI()
    {
        ...
        m_MaxVerticesPerObject = EditorGUILayout.IntSlider("Max Vertices", m_MaxVerticesPerObject, 3, 64998);
        if (GUILayout.Button("Atlas and combine") && m_Target)
        {
            m_Target = DeepCloneMeshes(m_Target);
            CombineTexturesWithSameMaterial(m_Target);
            CombineMeshes(m_Target);
        }

        EditorGUILayout.Separator();

        if (GUILayout.Button("Enable all meshes") && m_Target)
        {
            EnableAllMf(m_Target);
        }

        m_DecalCombiner = EditorGUILayout.BeginToggleGroup("Decal Combiner", m_DecalCombiner);

        m_surfaceMesh = (GameObject)EditorGUILayout.ObjectField("Surface", m_surfaceMesh, typeof(GameObject), true);
        m_decalMesh = (GameObject)EditorGUILayout.ObjectField("Decals", m_decalMesh, typeof(GameObject), true);
        m_decalShader = (Shader)EditorGUILayout.ObjectField("Shader", m_decalShader, typeof(Shader), false);
        m_UVChannel = (UVChannels) EditorGUILayout.Popup("UV channel for decal", (int) m_UVChannel, Enum.GetNames(typeof(UVChannels)));

        if (GUILayout.Button("Merge") && m_surfaceMesh && m_decalMesh && m_decalShader)
        {
            Vector3 center = m_surfaceMesh.GetComponent<MeshRenderer>().bounds.center;
            Vector3 size = m_surfaceMesh.GetComponent<MeshRenderer>().bounds.extents;
            GameObject obj = GameObject.CreatePrimitive(PrimitiveType.Plane);
            obj.transform.position = new Vector3(center.x, center.y + 10, center.z);
            obj.transform.localScale = new Vector3(size.x/m_surfaceMesh.transform.localScale.x, 1, size.z/ m_surfaceMesh.transform.localScale.y);

            Texture2D tex = new Texture2D(1024, 1024);
            obj.GetComponent<MeshRenderer>().sharedMaterial.mainTexture = tex;
    ...
    public void CombineMeshes(GameObject target)
    {
        ...
        Grid<MeshFilter> grid = PlaceObjectsInGrid(target, (int)m_GridSize.x, (int)m_GridSize.z, (int)m_GridSize.y);

        int j = 0;
        GameObject clone = target;
        //each grid item
    ...
    private static void MergeTextures(List<Texture2D> textureBatch, List<List<TextureTriangles>> objectsBatch, Shader shader)
    {
        ...
        foreach (Texture2D texture in textureBatch)
        {
            ...
            name += "_"+texture.name.Substring(0, 2);
        }


        Rect[] rects = combinedTexture.PackTextures(textureBatch.ToArray(), 2);

        Texture2D finalTex = ExportTexture(combinedTexture, "Assets/" + name + "_atlas.png");
        Debug.Log("created atlas " + name);
